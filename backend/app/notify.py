...
def main(argv: list[str] | None = None) -> int:
    ...
    # **보낼 것이 없어도 한 줄은 보낸다.**
    #
    # 예전에는 여기서 조용히 끝냈다. 알림이 안 울려서 깔끔해 보였는데,
    # 받는 쪽에서는 "오늘 조용한 날" 과 "고장 난 날" 이 똑같이 보인다.
    # 실제로 배치가 사흘 멈춘 걸 아무도 몰랐고 "오늘은 텔레그램 안
    # 왔는데" 로 알게 됐다.
    #
    # 내용이 없을 때 render_digest 는 "오늘은 새로 확인된 공식 발표가
    # 없습니다" 한 줄을 만든다. 그걸 보낸다. 사장님 안내는 안 보낸다 —
    # 전달할 것이 없는데 전달용 메시지를 만들 이유가 없다.
    if not cards:
        print("\n보낼 내용이 없습니다. 그 사실만 한 줄 보냅니다.")
