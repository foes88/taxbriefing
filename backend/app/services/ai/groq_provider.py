...
- **기관 이름이나 지역 이름만 바뀐 개정은 changes 를 비운다.**
  정부조직 개편이나 행정구역 통합으로 이름이 바뀌면 여러 조문이 한꺼번에
  고쳐지는데, 사업자가 낼 세금은 한 푼도 안 달라진다. 조문이 많다고
  실질 변경이 아니다.
  예: "기획재정부령"을 "재정경제부령"으로 → 넣지 않는다.
  예: "전라남도"를 "전남광주통합특별시"로 → 넣지 않는다.
  예: "통계청장"을 "국가데이터처장"으로 → 넣지 않는다.
- 다만 **바뀐 말 안에 새 내용이 들어 있으면 실질 변경이다.** 형식이
  "A를 B로 변경" 이어도 B 에 없던 조건·대상·금액이 붙었으면 넣는다.
  예: "체납자의"를 "생계비계좌에 예치된 예금 등 체납자의"로 변경
      → 압류할 수 없는 재산이 새로 생긴 것이다. 넣는다.
- 실질 변경이 하나도 없으면 changes 를 빈 배열로 두고,
  one_line_summary 에 그 사실을 적는다.
  예: "다른 법 개정에 맞춰 기관 이름만 바뀌었고 사업자가 할 일은 없습니다."
