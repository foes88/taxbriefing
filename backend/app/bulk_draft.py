...
def _preannounce_body(meta: dict) -> dict:
    ...
    **법령 틀을 쓰면 정반대가 된다.** 그냥 두면 이렇게 나갔을 것이다.
    ...
    return {
        ...
        # 마감 날짜를 값으로도 넣는다.
        #
        # 요약 문장에만 두면 화면이 "오늘 기준 며칠 남았는가" 를 셀 수 없다.
        # 8월 20일이 지나도 「의견 제출은 8월 20일까지입니다」 가 그대로
        # 떠 있게 되는데, 지난 기한을 앞으로의 기한처럼 보여주는 것이다.
        #
        # 콘텐츠 컬럼(application_end)에는 넣지 않는다. 그 칸은 "제도가
        # 적용되는 기간" 이라 뜻이 다르고, 일정 화면이 시행 예정으로 읽는다.
        "comment_deadline": closes or None,
