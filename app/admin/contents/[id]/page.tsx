...
import type { AdminContent, GateReport, LegalStatus, RiskLevel } from '@/lib/types';
...
export default function ContentEditorPage() {
  ...
  const load = useCallback(async () => {
    const [c, g] = await Promise.all([adminApi.content(id), adminApi.gates(id)]);
    setContent(c);
    setGates(g);
      ...
      <section className="mt-4 border border-rule bg-surface p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button
            type="button"
            onClick={approve}
            disabled={busy || !gates?.can_approve}
            title={gates?.can_approve ? undefined : '게이트를 통과하지 못했습니다'}
